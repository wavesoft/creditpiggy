...
class JSONPProtocol(APIProtocol):
	"""
	JSON-P Implementation of the CreditPiggy API Protocol
	...
	def build_jsonp_message(self, data, callback):
		"""
		Compile a simple HTML page that just fires the specified callback with the data given.
		"""

		# Return just a script that posts the message to parent or opener
	...
	def render_success(self, data):
		...
		res = HttpResponse(
				self.build_jsonp_message(response, self.request.GET.get('callback', 'jsonp_callback')), 
